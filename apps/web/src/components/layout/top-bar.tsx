...
import { useProfileStore, ROLE_META, PROFILE_REGIONS } from "@/stores/profile-store";
...
export function TopBar() {
  ...
  const regionLabel = profile
    ? (isAr
        ? (PROFILE_REGIONS.find((r) => r.key === profile.region)?.labelAr ?? profile.region)
    ...
    <header className="no-print flex shrink-0 flex-col border-b border-white/[0.06] bg-atlas-bg-secondary">
      ...
      <div className="flex h-12 items-center justify-between">
        ...
        <div className="flex items-center gap-2 pr-5">
          ...
          {/* Profile badge with Edit or setup button */}
          {profile && roleMeta ? (
            <div className="flex items-center gap-1">
              <button
                onClick={openModal}
                className="flex items-center gap-1.5 px-2.5 py-1 border border-atlas-accent/20 bg-atlas-accent/[0.05] hover:border-atlas-accent/40 transition-colors cursor-pointer"
